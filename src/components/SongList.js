...
class SongList {
  ...
  setVolume(vol) {
    ...
    // Update the cached song volume if the passed value is valid
    const element = this.songPlaying;
    if (element !== null) {
      element.volume = vol;
    }
    ...
    for (let i = 0; i < totalSongs; i += 1) {
      ...
      tempAudio.addEventListener('loadedmetadata', () => {
        ...
        if (songNodes.length === totalSongs) {
          ...
          for (let node of songNodes) {
            this.elemList.appendChild(node);
          }
  ...
  createSongNode(songPath, durationRaw) {
    ...
    // Append name and artist nodes to the song container
  ...
  updateHeader() {
    ...
    // Add the 'Path:' to header regardsless of existing music path
  ...
  updateFooter() {
    ...
    elemFooter.appendChild(volNode);
    elemFooter.appendChild(randomNode);
  }
}
